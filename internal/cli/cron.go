...
func runCron(ctx context.Context, args []string, deps Dependencies) int {
	...
	case "show":
		...
		if err != nil {
			fmt.Fprintf(deps.Stderr, "cron show: %v\n", err)
			return 3
		}
	...
	case "remove":
		...
		if err := deps.Cron.Remove(); err != nil {
			fmt.Fprintf(deps.Stderr, "cron remove: %v\n", err)
			return 3
		}
		fmt.Fprintln(deps.Stdout, "cron schedule removed")
		return 0
	default:
		fmt.Fprintf(deps.Stderr, "unknown cron command %q\n", args[0])
		printCronUsage(deps)
		return 2
	}
}
...
func runCronInstall(ctx context.Context, args []string, deps Dependencies) int {
	...
	if err != nil {
		fmt.Fprintf(deps.Stderr, "cron install validation: %v\n", err)
		return 2
	}
	...
	if err != nil {
		fmt.Fprintf(deps.Stderr, "cron install: render schedule: %v\n", err)
		return 2
	}
	if err := deps.Cron.Install(content); err != nil {
		fmt.Fprintf(deps.Stderr, "cron install: %v\n", err)
		return 3
	}
